...
def plot_viscosity(
    df: pd.DataFrame,
    color_by: str = "source_doi",
    colors: list[str] | dict[str, str] | None = None,
    marker_by_group: str | None = None,
    marker_by_doi: bool = False,
    marker_styles: list[str] | None = None,
    water_column: str = "mole_fraction_water",
    viscosity_column: str = "viscosity_value",
    ...
    alpha: float = 0.9,
    cmap: str = "tab20",
    water_round_decimals: int = 3,
    legend_title: str | None = None,
    show_temperature_top_axis: bool = True,
    top_axis_ticks: int = 6,
    show_errorbars: bool = True,  # <-- new argument
):
    """
    Plot ln(viscosity) vs 1000/(R*T) with configurable grouping/coloring.

    Designed for DataFrames produced by FAIRFluids extraction utilities (e.g.
    extract_property_dataframe for viscosity).

    Args:
        df: Input DataFrame with viscosity and temperature columns.
        color_by: Group/color column. Typical values:
            - "source_doi"
            - "mole_fraction_water"
            - "composition"
            - "composition_and_doi"
        colors: Optional custom colors for plot groups.
            - If None: use standard behavior (existing defaults).
            - If dict: keys are group values, values are matplotlib colors.
            - If list: colors are assigned in first-appearance order of groups.
        marker_by_group: Optional marker grouping key. If set, assign distinct
            marker styles per group value. Supports:
            - existing columns (e.g. "source_doi", "method")
            - "composition"
            - "composition_and_doi"
        marker_by_doi: Backward-compatible alias for marker_by_group="source_doi".
            Ignored if marker_by_group is explicitly provided.
        marker_styles: Optional list of marker styles used for marker-group mapping.
        water_column: Name of water mole fraction column.
        viscosity_column: Name of viscosity values column.
        temperature_column: Name of temperature column.
        figsize: Matplotlib figure size.
        point_size: Marker size.
        edgecolor: Marker edge color.
        linewidth: Marker edge linewidth.
        alpha: Marker transparency.
        cmap: Matplotlib colormap name.
        water_round_decimals: Rounding precision when grouping by water content.
        legend_title: Optional custom legend title.
        show_temperature_top_axis: If True, add top x-axis with temperature [K].
        top_axis_ticks: Number of ticks on top temperature axis.
        show_errorbars: If True, will plot errorbars if present (default True).

    Returns:
        tuple: (fig, ax, plot_df)
            - plot_df: filtered/derived plotting DataFrame
    """
    def _string_to_hex_color(s: str, sat: float = 0.65, val: float = 0.85) -> str:
        """
        Deterministically map a string to a visually distinct hex color.
        Useful for stable DOI coloring across runs.
        """
    ...
    required_cols = [viscosity_column, temperature_column]
    if color_by in {"composition", "composition_and_doi"}:
        required_cols.append("fluid_compounds")
        if color_by == "composition_and_doi":
            required_cols.append("source_doi")
    else:
        required_cols.append(color_by)
    missing = [col for col in required_cols if col not in df.columns]
    if missing:
        raise ValueError(f"Missing required columns for plot_viscosity: {missing}")

    # Work on a copy and keep only rows with complete numeric essentials
    plot_df = df.copy()
    plot_df = plot_df[
        plot_df[viscosity_column].notnull() & plot_df[temperature_column].notnull()
    ].copy()
    if plot_df.empty:
        raise ValueError("No valid rows after filtering null viscosity/temperature.")

    needs_water_column = color_by == water_column

    # Build water fraction if needed but missing and raw columns exist
    if needs_water_column and water_column not in plot_df.columns:
        if "fluid_compounds" in plot_df.columns and "mole_fractions" in plot_df.columns:
            ...
            plot_df[water_column] = plot_df.apply(_extract_water_fraction, axis=1)
        else:
            raise ValueError(
                f"Column '{water_column}' not found and cannot be derived from "
                "'fluid_compounds' + 'mole_fractions'."
            )

    # Numeric conversions
    R = 8.314  # J/(mol*K)
    plot_df[temperature_column] = pd.to_numeric(
        plot_df[temperature_column], errors="coerce"
    )
    plot_df[viscosity_column] = pd.to_numeric(
        plot_df[viscosity_column], errors="coerce"
    )
    plot_df = plot_df[
        plot_df[temperature_column].notnull() & plot_df[viscosity_column].notnull()
    ].copy()
    plot_df = plot_df[plot_df[viscosity_column] > 0].copy()
    if plot_df.empty:
        raise ValueError("No valid rows with positive viscosity for log transform.")

    plot_df["inv_RT"] = 1000.0 / (R * plot_df[temperature_column].astype(float))
    plot_df["ln_viscosity"] = np.log(plot_df[viscosity_column].astype(float))

    # Add error columns for viscosity and ln(viscosity) if available
    # ---- Begin error analysis rewrite ----
    err_col_candidates = [col for col in plot_df.columns if 'error' in col.lower()]
    err_visc_col = None
    for candidate in err_col_candidates:
        # Use the first column that matches typical patterns
        if candidate.lower() in [
            "viscosity_error", "viscosity_err", "viscosity_uncertainty",
            f"{viscosity_column}_error", f"{viscosity_column}_err",
            f"{viscosity_column}_uncertainty", "error_viscosity"
        ]:
            err_visc_col = candidate
            break
    if err_visc_col is None:
        # fallback - try first column with both error and visc
        for candidate in err_col_candidates:
            if "visc" in candidate.lower():
                err_visc_col = candidate
                break

    plot_df["_ln_viscosity_err"] = None
    plot_df["_viscosity_err"] = None
    if err_visc_col and (err_visc_col in plot_df.columns):
        plot_df["_viscosity_err"] = pd.to_numeric(plot_df[err_visc_col], errors="coerce")
        # where possible, is the error already log transformed? Reasonable assumption: no
        # so we propagate error using delta(ln y) = delta(y) / y
        mask = (plot_df["_viscosity_err"] > 0) & (plot_df[viscosity_column] > 0)
        plot_df.loc[mask, "_ln_viscosity_err"] = (
            plot_df.loc[mask, "_viscosity_err"] / plot_df.loc[mask, viscosity_column]
        )
        # If you want to check if the error is already ln(viscosity) error,
        # you would need some convention or metadata; current logic ALWAYS
        # assumes linear error and propagates to log.
    # ---- End error analysis rewrite ----

    # Grouping key (discrete) for coloring/legend
    if color_by == water_column:
        plot_df["_plot_group"] = pd.to_numeric(plot_df[water_column], errors="coerce").round(
            water_round_decimals
        )
    elif color_by == "composition":
        plot_df["_plot_group"] = plot_df["fluid_compounds"].apply(
            lambda comps: " + ".join(sorted(map(str, comps)))
            if isinstance(comps, list)
            else "NA"
        )
    elif color_by == "composition_and_doi":
        plot_df["_plot_group"] = plot_df.apply(
            lambda row: (
                f"{' + '.join(sorted(map(str, row['fluid_compounds'])))} | "
                f"{row['source_doi'] if pd.notna(row['source_doi']) else 'NA'}"
            )
            if isinstance(row.get("fluid_compounds"), list)
            else "NA",
            axis=1,
        )
    else:
        plot_df["_plot_group"] = plot_df[color_by].fillna("NA")

    plot_df = plot_df[plot_df["_plot_group"].notnull()].copy()
    if plot_df.empty:
        raise ValueError(f"No rows available after applying color_by='{color_by}'.")

    # Backward compatibility: marker_by_doi -> marker_by_group="source_doi"
    if marker_by_group is None and marker_by_doi:
        marker_by_group = "source_doi"

    groups = sorted(plot_df["_plot_group"].unique())
    # Default behavior aligned with notebook usage:
    # - source_doi: deterministic hash colors
    # - mole_fraction_water: Blues colormap
    if colors is not None:
        if isinstance(colors, dict):
            ...
            if missing_groups:
                raise ValueError(
                    "Custom 'colors' dict is missing entries for groups: "
                    f"{missing_groups}"
                )
        else:
            if len(colors) == 0:
                raise ValueError("Custom 'colors' list must not be empty.")
            # Use first appearance order to map list colors to groups robustly.
            group_order = list(dict.fromkeys(plot_df["_plot_group"].tolist()))
            color_map = {
                grp: colors[i % len(colors)] for i, grp in enumerate(group_order)
            }
            # Ensure all groups are available even if grouping order changes.
            for grp in groups:
                if grp not in color_map:
                    color_map[grp] = colors[len(color_map) % len(colors)]
    elif color_by == "source_doi":
        # Standard DOI coloring: stable hash-based colors.
        color_map = {grp: _string_to_hex_color(grp) for i, grp in enumerate(groups)}
    elif color_by == water_column:
        cmap_obj = plt.get_cmap("Blues", len(groups))
        color_map = {grp: cmap_obj(i) for i, grp in enumerate(groups)}
    else:
        cmap_obj = plt.get_cmap(cmap, len(groups))
        color_map = {grp: cmap_obj(i) for i, grp in enumerate(groups)}

    if marker_by_group is not None:
        if marker_styles is None:
            marker_styles = ["o", "s", "^", "D", "v", "P", "X", "<", ">", "h", "*", "8"]

        if marker_by_group == "composition":
            plot_df["_marker_group"] = plot_df["fluid_compounds"].apply(
                lambda comps: " + ".join(sorted(map(str, comps)))
                if isinstance(comps, list)
                else "NA"
            )
        elif marker_by_group == "composition_and_doi":
            if "source_doi" not in plot_df.columns:
                raise ValueError(
                    "marker_by_group='composition_and_doi' requires column 'source_doi'."
                )
            plot_df["_marker_group"] = plot_df.apply(
                lambda row: (
                    f"{' + '.join(sorted(map(str, row['fluid_compounds'])))} | "
                    f"{row['source_doi'] if pd.notna(row['source_doi']) else 'NA'}"
                )
            ...
            if marker_by_group not in plot_df.columns:
                raise ValueError(
                    f"marker_by_group '{marker_by_group}' not found in DataFrame columns."
                )
            plot_df["_marker_group"] = plot_df[marker_by_group].fillna("NA").astype(str)

        marker_values = sorted(plot_df["_marker_group"].astype(str).unique())
        marker_map = {
            key: marker_styles[i % len(marker_styles)] for i, key in enumerate(marker_values)
        }

    fig, ax = plt.subplots(figsize=figsize)
    # Show errorbars if possible using yerr if uncertainties were found
    for grp in groups:
        subset = plot_df[plot_df["_plot_group"] == grp]
        if marker_by_group is not None:
            first_in_group = True
            for marker_key, marker_subset in subset.groupby("_marker_group"):
                # Use error bars if present and if flag is True
                ln_visc_errs = (
                    marker_subset["_ln_viscosity_err"] if "_ln_viscosity_err" in marker_subset.columns else None
                )
                # Is the error log transformed? YES, now, by propagation: delta_ln_eta = delta_eta / eta
                has_err = ln_visc_errs is not None and marker_subset["_ln_viscosity_err"].notnull().any()
                if show_errorbars and has_err:
                    ax.errorbar(
                        marker_subset["inv_RT"],
                        marker_subset["ln_viscosity"],
                        yerr=marker_subset["_ln_viscosity_err"],
                        fmt=marker_map[str(marker_key)],
                        color=color_map[grp],
                        markersize=np.sqrt(point_size),  # area to diameter
                        markeredgecolor=edgecolor,
                        markeredgewidth=linewidth,
                        alpha=alpha,
                        linestyle="None",
                        label=str(grp) if first_in_group else None,
                    )
                else:
                    ax.scatter(
                        marker_subset["inv_RT"],
                        marker_subset["ln_viscosity"],
                        color=color_map[grp],
                        marker=marker_map[str(marker_key)],
                        s=point_size,
                        edgecolor=edgecolor,
                        linewidth=linewidth,
                        alpha=alpha,
            ...
            ln_visc_errs = (
                subset["_ln_viscosity_err"] if "_ln_viscosity_err" in subset.columns else None
            )
            has_err = ln_visc_errs is not None and subset["_ln_viscosity_err"].notnull().any()
            if show_errorbars and has_err:
                ax.errorbar(
                    subset["inv_RT"],
                    subset["ln_viscosity"],
                    yerr=subset["_ln_viscosity_err"],
                    fmt="o",
                    color=color_map[grp],
    ...
    if legend_title is None:
        legend_title = (
            ...
            if color_by == "source_doi"
            else "Composition + DOI"
            if color_by == "composition_and_doi"
            else "Composition"
            if color_by == "composition"
            else str(color_by)
        )
    ...
    ax.legend(title=legend_title, bbox_to_anchor=(1.04, 1), loc="upper left")

    if show_temperature_top_axis:
        # Top axis should display temperature corresponding to bottom inv_RT axis.
        # Since inv_RT = 1000 / (R*T), temperature decreases as inv_RT increases.
        ax_top = ax.twiny()
        x_min, x_max = ax.get_xlim()
        eps = 1e-16
        x_min_safe = x_min if abs(x_min) > eps else eps
        x_max_safe = x_max if abs(x_max) > eps else eps

        # Convert inv_RT bounds to temperature bounds [K].
        t_left = 1000.0 / (R * x_min_safe)   # corresponds to left side (usually higher T)
        t_right = 1000.0 / (R * x_max_safe)  # corresponds to right side (usually lower T)

        ax_top.set_xlim(t_left, t_right)
        tick_count = max(2, int(top_axis_ticks))
        temp_ticks = np.linspace(t_left, t_right, tick_count)
        ax_top.set_xticks(temp_ticks)
        ax_top.set_xticklabels([f"{t:.0f}" for t in temp_ticks])
        ax_top.set_xlabel("Temperature [K]")

    plt.tight_layout()

    drop_columns = ["_plot_group"]
    if "_marker_group" in plot_df.columns:
        drop_columns.append("_marker_group")
    # Drop _ln_viscosity_err and _viscosity_err temporary columns if they exist
    for extra_col in ["_ln_viscosity_err", "_viscosity_err"]:
        if extra_col in plot_df.columns:
            drop_columns.append(extra_col)
    return fig, ax, plot_df.drop(columns=drop_columns)
...
def _extract_measurements_with_all_parameters(
    ...
    def _get_fluid_measurements(fluid_obj):
        """Return measurements from new or legacy fluid structure."""
        sample = getattr(fluid_obj, "sample", None)
        if sample is not None and getattr(sample, "measurement", None) is not None:
            return sample.measurement
        return getattr(fluid_obj, "measurement", []) or []
    ...
    for fluid in doc.fluid:
        ...
        # Process measurements from both supported structures:
        # - new schema: fluid.sample.measurement
        # - legacy schema: fluid.measurement
        for measurement in _get_fluid_measurements(fluid):
            ...
            for param_val in measurement.parameterValue:
                ...
                if param_def is not None:
                    # New schema uses `parameters`; legacy payloads may use `parameter`.
                    param_obj = getattr(param_def, "parameters", None)
            ...
            # Normalize mole fractions when available and complete.
            if mole_fractions and None not in mole_fractions:
                mole_frac_sum = sum(mole_fractions)
                if mole_frac_sum > 0:
                    normalized_mole_fractions = [
                        mf / mole_frac_sum for mf in mole_fractions
            ...
            # Process each property value regardless of temperature/composition completeness.
            for prop_val in measurement.propertyValue:
                pdef = property_lookup.get(prop_val.propertyID)
                ...
                if property_types is not None and resolved_type not in property_types:
                    continue
...
__all__ = [
    ...
    "get_activation_energies_dataframe",
